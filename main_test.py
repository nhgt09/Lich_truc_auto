...
def luu_ds_hoc_sinh(duong_dan="danhsach.json"):
    """Lưu dữ liệu học sinh ra file JSON."""
    global DATA_HS
    with open(duong_dan, "w", encoding="utf-8") as f:
        json.dump(DATA_HS, f, ensure_ascii=False, indent=2)
...
def mo_file_word():
    ...
    try:
        ...
        for para in doc.paragraphs:
            ...
            if not text:
                continue
            if text.startswith("Tổ"):
                to_hien_tai = text
                DATA_HS[to_hien_tai] = []
            elif " " in text and to_hien_tai:
                ten, gioi = [s.strip() for s in text.split(" ", 1)]
                DATA_HS[to_hien_tai].append({"ten": ten, "gioi_tinh": gioi})

        # Lưu ra JSON
        with open("danhsach.json", "w", encoding="utf-8") as f:
            json.dump(DATA_HS, f, ensure_ascii=False, indent=2)

        # ✅ Cập nhật OptionMenu tổ
        if "bien_to" in WIDGETS and "optionmenu_to" in WIDGETS:
            menu = WIDGETS["optionmenu_to"]["menu"]
            menu.delete(0, "end")
            for to in DATA_HS.keys():
                menu.add_command(
                    label=to,
                    command=lambda v=to: cap_nhat_hoc_sinh_theo_to(v)
                )
            # chọn tổ đầu tiên làm mặc định
            first_to = list(DATA_HS.keys())[0]
            WIDGETS["bien_to"].set(first_to)
            cap_nhat_hoc_sinh_theo_to(first_to)

        messagebox.showinfo("Thành công", "Đọc file Word và cập nhật giao diện thành công")
