     ...
     try:
          ...
          for dulieu_ch_xl in file.paragraphs:
               ...
               if dulieu.lower().startswith("tổ"):
                    ...
                    if len(parts) == 2:
                         ten, gioi_tinh = parts[0].strip(), parts[1].strip().lower()
                         DANHSACH_HS_LUU[danh_sach_to].append({"ten": ten, "gioi_tinh": gioi_tinh})
...
def trich_xuat_file_danhsach():
     ...
     with open(file_danh_sach, "r", encoding="utf-8") as f:
          data  = json.load(f)
     toor = list(data.keys())
     for to, danh_sach_hs in data.items():
          tam = []
          for hoc_sinh in danh_sach_hs:
               ten = hoc_sinh.get("ten", "")
               gioi_tinh = hoc_sinh.get("gioi_tinh", "").lower()
               gt = 0 
               if gioi_tinh == "nữ":
                    gt =0
               else:
                    gt = 1
               tam.append([ten, gt])
               DANHSACH_HS.append(tam)
          DANHSACH_HS_TUNG_TO.append(tam)
...
#=========================MAIN=========================
     ...
     test = [hs[0] for to in DANHSACH_HS for hs in to]
